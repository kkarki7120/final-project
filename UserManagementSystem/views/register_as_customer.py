...
class RegisterCustomer(View):
    def get(self, request):
        ...
        return render(request , 'register_as_customer.html',loc)
    ...
    def post(self, request):
        ...
        if usertype == "Renter":
            #Renter user object
            user = CustomUser(profile_pic = profile_picc , fullname = fullname , username = username , phone = phone , email = email , user_type = usertype , address = user_address , password = password)
            
            #saving user entered value in dictionary
            entered_value ={'profile_pic':profile_picc,'fullname':fullname ,'username': username ,'phone':phone ,'email' :email,"loc":location, address : user_address}
            ...
            if (not error_msg):
                ...
                data = {'error':error_msg ,'values':entered_value,"loc":location}        
                return render(request , 'register_as_customer.html' , data)
        else:
            return render(request , 'register_as_vendor.html' )
